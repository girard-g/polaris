...
- **Windows hook shell (partially verified).** `polaris setup` now quotes the hook command per platform: POSIX single-quoting via `shell-words` on macOS/Linux, unconditional double-quoting on Windows (`"C:\Program Files\polaris\polaris.exe" hook index`). Double quotes are what `cmd.exe` honors — POSIX single quotes it passes through literally — and they also round-trip through `shell_words::split`, so `is_polaris_owned` still recognizes the entry on any host. What remains unverified end-to-end is **which** shell Claude Code on Windows dispatches hooks through: under PowerShell a quoted string in command position is parsed as a string expression and would need a leading `& `, which the hook config (a bare `command` string, no args array) gives us no way to express conditionally. If hooks silently fail to fire on a Windows install, that is the suspect. A path without spaces (e.g. `C:\tools\polaris\polaris.exe`) sidesteps it under either shell.
