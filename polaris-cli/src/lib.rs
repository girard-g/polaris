...
/// Parse args and dispatch every command. The public entry point used by the
/// `polaris` binary and, in the Pro build, by `polaris-pro` after it installs
/// its ingestion hook.
#[tokio::main]
pub async fn run() -> std::process::ExitCode {
    ...
    // Decide notice visibility from the parsed command before `dispatch` consumes it.
...
async fn dispatch(cli: Cli) -> Result<()> {
    ...
    match cli.command {
        ...
        Command::UpdateRefresh => unreachable!("handled in run"),
    }
}
